...
{
	private $_model;
	private $_asFields 				= array();
	private $_asWhere 				= array();
	private $_asOrderBy				= array();
	private $_asLimit				= array();
	private $_asTables				= array();
	private $_asJoin				= array();
	private $_sGroupBy				= '';
	private $_customInstruction                     = '';
	private $_debug				 	= false;
	private $_count				 	= false;
	private $_prefix			 	= '';
	private $_prefixEnabled                         = true;
...
//                die();
	...
	{
            if (is_array($psField)) return $this->addFields($psField);

            if (empty($psField)) $psField = ' * ';
	...
	{
            if (empty($pnLength) || $pnLength < 0) throw new \ErrorException('$pnLength must be > 0');
            if ($pnStart < 0) throw new \ErrorException('$pnStart must be >= 0');
...
//                utils\Debug::e($oDbResponse);
	...
	{
            if (is_integer($this->_count)) return $this->_count;
	...
	{
            if (empty($this->_asFields)) return ' * ';
...
//                        $asTables[] = $sTable;
	...
	{
            if (!empty($this->_sWhere)) return $this->_sWhere;
